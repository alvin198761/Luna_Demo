...
class endpoint {
	...
	def list(InvocationContext context) {
		...
		context.attributes['factory'].getBeansOfType(Endpoint.class).each { name, endpoint ->
	...
	def invoke(InvocationContext context, @Usage("The name of the Endpoint to invoke") @Required @Argument String name) {
		...
		context.attributes['factory'].getBeansOfType(Endpoint.class).each { n, endpoint ->
			if (n.equals(name) && endpoint.isEnabled()) {

				EndpointMBean mbean = context.attributes['factory'].getBean(EndpointMBeanExporter.class).getEndpointMBean(name, endpoint)
